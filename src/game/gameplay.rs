...
use card_game_engine::game::{Game, GameBuilder, GameRunner, GameState, DefaultGameState};
use card_game_engine::moves::{GameMove, DefaultMove};
...
impl GameMove<DDPState> for PlayerMove {
    ...
    fn handle_move(&self, game: &mut DDPState) -> Result<(), Self::E> {
        match self {
            PlayerMove::Draw => if let Err(e) = DefaultMove::handle_move(&DefaultMove::Draw, &mut game.default_state) {
                return Err(e.into());
            }
            PlayerMove::Discard(c) => if let Err(e) = DefaultMove::handle_move(&DefaultMove::Discard(*c), &mut game.default_state) {
                return Err(e.into());
            },
            PlayerMove::TakeDiscardPile => {
                let turn = game.default_state.turn;

                let mut player_hand_with_discard = game.default_state.players[game.default_state.turn].hand.clone();
                ...
                ) { return Err(DameDePiqueError::InvalidDiscardOpeningHand(game.default_state.turn)); }
            ...
            PlayerMove::Open(cards) => {
                ...
                if !PlayerMove::hand_can_open(
                    game.get_partners_from_player(turn).who_opened(turn),
                    &cards
                ) { return Err(DameDePiqueError::InvalidOpeningHand(game.default_state.turn));}
                game.get_partners_from_player(turn).add_points(cards.to_vec());
                game.get_partners_from_player(turn).update_status(turn);
            },
            PlayerMove::AddPoints(cards) => {
                let turn = game.default_state.turn;
                if !PlayerMove::can_add_points(turn, game) { return Err(DameDePiqueError::PlayerCantAddPoints(game.default_state.turn)); }
                if !game.get_partners_from_player(game.default_state.turn).are_valid_points(&cards) { return Err(DameDePiqueError::InvalidPoints); }
                game.get_partners_from_player(game.default_state.turn).add_points(cards.to_vec());
            },
        }
        Ok(())
    }
